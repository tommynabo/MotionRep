...
Deploys complete video processing infrastructure in one go
"""
...
def check_prerequisites():
    """Verify all files and tools are ready"""
    print("=" * 70)
    print("CHECKING PREREQUISITES")
    print("=" * 70)
    
    checks = [
        (LAYER_ZIP.exists(), f"Lambda Layer ZIP exists: {LAYER_ZIP}"),
        ((LAMBDA_DIR / "index.py").exists(), f"Lambda handler exists: {LAMBDA_DIR / 'index.py'}"),
        ((LAMBDA_DIR / "detect_exercise_lambda.py").exists(), f"MediaPipe script exists"),
    ]
    ...
    for passed, desc in checks:
        ...
        print(f"{status} {desc}")
        if not passed:
            raise Exception(f"Prerequisites not met: {desc}")

def publish_lambda_layer():
    """Upload Lambda Layer to AWS"""
    print("\n" + "=" * 70)
    print("STEP 1: PUBLISH LAMBDA LAYER")
    print("=" * 70)
    
    layer_zip_size = LAYER_ZIP.stat().st_size / (1024**2)
    print(f"Layer ZIP size: {layer_zip_size:.1f}MB")
    ...
    output = run_command(cmd, "Publishing Lambda Layer", capture=True)
...
def create_lambda_function(layer_arn):
    ...
    run_command(
        f"cd {package_dir} && zip -r -q {zip_path} . && cd -",
...
def main():
    try:
        ...
        print(f"  LAMBDA_WEBHOOK_SECRET=your-webhook-secret")
