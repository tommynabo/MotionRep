#!/usr/bin/env python3
"""
Pose Detection for Exercise Range Identification
Uses MediaPipe to detect when exercise motion starts and ends in a video.
Filters out setup/adjustment frames at the beginning and end.
"""
...
def detect_exercise_range(video_path, min_movement_threshold=0.08, window_size=5):
    ...
    return {
        ...
        'fps': float(fps),
        'frames_data': frames_data  # For debugging
    }
...
def find_exercise_range(frames_data, min_threshold=0.08):
    ...
    Filters out:
    - Beginning: Setup/adjustment phase (low movement, person is positioning)
    - End: Cool-down or repetition wind-down (fading movement)
    
    Strategy:
    1. Find first frame with significant smoothed movement
    2. Find last frame with significant smoothed movement
    3. Optionally trim edges if movement drops below threshold
    """
    if not frames_data:
        return None, None
    ...
    # Optionally: look back a few frames to catch the beginning of motion
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        error = {'error': 'Missing video_path argument', 'usage': 'python detect_exercise.py <video_path>'}
    ...
    try:
        ...
        # Remove frames_data from JSON output (too large)
        if 'frames_data' in result:
            del result['frames_data']
        print(json.dumps(result))
    except Exception as e:
        error = {'error': str(e), 'video_path': video_path}
